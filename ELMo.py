...
from char_ebmed import CharEmbedding

class ELMo(nn.Module):
    def __init__(self,
                 vocab_size, output_dim: int, kernel_sizes: List[List[int]], device: str,
                 emb_dim: int = 16, hid_dim: int = 512, prj_dim: int = 512,
                 seq_len: int = 50, n_layers: int = 2, dropout: float = 0.):
        r"""
        Parametrs:
            vocab_size: Character vocabulary size
            output_dim: Word vocabulary size -> @TODO: Character vocabulary size
        ...
        self.embedding = CharEmbedding(vocab_size, emb_dim, prj_dim, kernel_sizes, seq_len, device=device)
    ...
    def forward(self, x: torch.Tensor):
        r"""
        Parameters:
            x: Sentence -> @TODO: Word
        Dimensions:
            x: [batch, seq_len] -> @TODO: Word length
        """
