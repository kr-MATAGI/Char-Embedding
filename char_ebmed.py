...
class CharEmbedding(nn.Module):
    ...
    """
    def __init__(self,
                 vocab_size: int, emb_dim: int, prj_dim: int,
                 kernel_sizes: List[List[int]], char_len: int, device: str):
    ...
    def forward(self, x):
        ...
        Dimensions:
            x: [Batch, Seq_len, Char_len] @TODO: [Batch, Word_len, Char_len]
        """
        batch_size, seq_len, _ = x.size()
        y = torch.zeros(batch_size, seq_len, self.kernel_dim).to("cpu")  # [Batch, Seq_len, Kernel_dim]
...
class CharCNN(nn.Module):
    ...
    def __init__(self, vocab_size: int, char_emb_dim: int, word_emb_dim: int, kernel_sizes: List[List[int]],
                 char_len: int, device: str):
        r"""
        Parameters:
        ...
        self.kernels = nn.ModuleList(
            [nn.Conv1d(in_channels=char_emb_dim, out_channels=num_features, kernel_size=kernel_size)
             for kernel_size, num_features in
             kernel_sizes])  # [Batch, Emb_dim, char_len] -> # [Batch, 1, Char_len - Kernel_size + 1]
    ...
    def forward(self, word: torch.Tensor):
        ...
        Dimensions:
            input:
                word: [Batch, Emb_dim, Seq_len] -> @TODO: [Batch, Emb_dim, Word_len]
            output:
                y:[Batch, Kernel_dim]
        """
        batch_size = word.size(0)
        print("AAAA: ", word.shape)
        y = torch.zeros(batch_size, self.word_emb_dim).to("cpu")  # [Batch, Kernel_dim]
